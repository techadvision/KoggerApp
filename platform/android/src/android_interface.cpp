...
jclass getActivityClass()
{
    ...
    if (!javaClass) {
        ...
        if (!QJniObject::isClassAvailable(kJniPulseActivityClassName)) {
        //if (!QJniObject::isClassAvailable(kJniKoggerActivityClassName)) {
            qCWarning(AndroidInterfaceLog) << "Class Not Available";
            return nullptr;
        }

        javaClass = env.findClass(kJniPulseActivityClassName);
...
QString getSDCardPath()
{
    ...
    const QJniObject result = QJniObject::callStaticObjectMethod(kJniPulseActivityClassName, "getSDCardPath", "()Ljava/lang/String;");
    if (!result.isValid()) {
