...
mosaic::PlotColorTable::PlotColorTable()
    ...
    , highLevel_(100.0f)
{
    setTheme(static_cast<int>(ThemeId::ClassicTheme));
...
void mosaic::PlotColorTable::setTheme(int id)
{
    ...
    */
    ...
    /* TODO Consider these new options
    ...
    */
    ...
    case ThemeId::HQthemeOrange: { // PULSE TRIAL: ported from echogram HQthemeOrange (id 26)
                                   // so the side-scan "High Quality Orange" theme matches in 3D.
        colors = {
                  QColor::fromRgb(0,0,0),
        ...
        // 256 levels evenly spaced 0..255 (mirrors the echogram palette).
        levels.resize(256);
        for (int i = 0; i < levels.size(); ++i) { levels[i] = i; }
        break;
    }
